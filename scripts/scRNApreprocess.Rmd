...
#dir.create(paste(directory_out,"Preprocessing",sep="/"), showWarnings = FALSE)
...
for (i in 1:dim(run2process)[1]){
	...
	if (i > 1){
		if (as.character(run2process[i,2]) == as.character(seuratObj_ls[[counter]]@project.name)){
			#if current sampleID is the same as the sampleID in the list then merge the objects
			seuratObj_ls[[counter]] <- merge(seuratObj_ls[[counter]],new_seurat_Obj, project = as.character(run2process[i,2]))
			
		} else {
			#if not duplicate then increment counter
			counter <- counter + 1
			seuratObj_ls[[counter]] <- new_seurat_Obj
		}
	} else {
		seuratObj_ls[[counter]] <- new_seurat_Obj
	}
	for (k in 3:ncol(run2process)){
		seuratObj_ls[[counter]][[as.character(colnames(run2process)[k])]] <- rep(as.character(run2process[i,k]),nrow(seuratObj_ls[[counter]]@meta.data))
...
for (i in 1:length(seuratObj_ls)){
...
fresh_Feat <- median(seuratObj_ls[[1]][["nFeature_RNA"]]$nFeature_RNA)
fresh_count <- median(seuratObj_ls[[1]][["nCount_RNA"]]$nCount_RNA)
fresh_cell <- dim(seuratObj_ls[[1]])[2]
fresh_features <- dim(seuratObj_ls[[1]])[1]

if (integrate_samples){
	cultured_Feat <- median(seuratObj_ls[[2]][["nFeature_RNA"]]$nFeature_RNA)
	cultured_count <- median(seuratObj_ls[[2]][["nCount_RNA"]]$nCount_RNA)
	cultured_cell <- dim(seuratObj_ls[[2]])[2]
	cultured_features <- dim(seuratObj_ls[[2]])[1]

	metrics <- data.frame(row.names = c("Fresh","Cultured"), median_genes_per_cell = c( fresh_Feat, cultured_Feat), cell_count = c(fresh_cell,cultured_cell), median_UMI_per_cell = c(fresh_count,cultured_count), features = c(fresh_features,cultured_features))
}
kable(metrics)
```
...
if(integrate_samples){
	...
	#reference_dataset <- c(1,2) #use if setting up a reference dataset(s)


	seurat.anchors <- FindIntegrationAnchors(object.list = seuratObj_ls, normalization.method = "SCT", anchor.features = seuratFeatures, verbose = FALSE) # Add ", reference = reference_dataset" if useing reference_dataset

	all.integrated <- IntegrateData(anchorset = seurat.anchors, normalization.method = "SCT", verbose = FALSE)
} else {
	all.integrated <- FindVariableFeatures(seuratObj_ls[[1]], selection.method = "vst", nfeatures = 2000)
}
```
...
all.integrated <- RunPCA(integrated, npcs = 50, verbose = FALSE)
